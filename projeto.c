...
struct Cursos *og_cursos(char *file_cursos, char *vagas_curso_comando)
{
    ...
    if (file_cursos == NULL)
    {
        file_cursos = "Cursos_Completo.csv";
    }
...
struct node *og_candidatos(char *file_candidatos, char *n_candidatos_comando)
{
    ...
    {
        file_candidatos = "Candidatos_Completo.csv";
    }
    ...
    while (inicio_node_ordenado_nota != NULL)
    {
        ...
        for (int l = 0; l < 5; l++)
        {
            ...
            while (curso_atual != NULL)
            {
                ...
                if ((atoi(inicio_node_ordenado_nota->candidato->opcoes[l].ide) == curso_atual->id_inst) && (strcmp(inicio_node_ordenado_nota->candidato->opcoes[l].idc, curso_atual->id_curso) == 0) && (curso_atual->vagas_sobram > 0))
                {
                ...
                else if ((atoi(inicio_node_ordenado_nota->candidato->opcoes[l].ide) == curso_atual->id_inst) && (strcmp(inicio_node_ordenado_nota->candidato->opcoes[l].idc, curso_atual->id_curso) == 0) &&
...
void file_colocacoes(struct Cursos *inicio_cursos, char *file)
{
    ...
    while (curso_atual != NULL)
    { // loop que percorre os cursos

        struct node *colocados = curso_atual->candidatos_colocados;
        while (colocados != NULL)
        { // loop que percorre a lista de colocados nesse curso e que imprime os respetivos dados para o ficheiro de saida escolhido
...
void file_completo(struct Cursos *inicio_cursos, char *file)
{
    ...
    struct Cursos *curso_atual = inicio_cursos;
    while (curso_atual != NULL)
    { // loop que percorre a lista de cursos e que imprime os respetivos dados para o ficheiro de saida escolhido
...
void file_nao_colocados(struct node *inicio_node_ordenado_ID, char *file)
{
    ...
    while (inicio_node_ordenado_ID != NULL)
    { // loop que percorre a lista de candidatos ordenados por ordem ascendente de ID

        if (inicio_node_ordenado_ID->candidato->colocacao == 0)
        { // imprime dados apenas dos nao colocados para o ficheiro de saida escolhido
...
int main(int argc, char *argv[])
{
    ...
    while ((n = getopt(argc, argv, "v:n:i:c:o:u:m:x:h")) != -1)
    {
        ...
        case 'n':
            ...
            if (atoi(n_candidatos_comando) < 0)
            { // o número de candidatos não pode ser negativo
